...
class Timetable2(BasicTimetable):
    ...
    def overlapsBreak(self, term: Term):
        ...
        if type(lesson) is not Lesson:
            ...
            for les in list(self.lesson_dict.values()):
                if self.busy(lesson.term):
                    raise ValueError(
                        f'Podany termin jest zajęty przez inną lekcję, nie mogę dodać lekcji {lesson.name}.')

            if self.overlapsBreak(lesson.term):
                raise ValueError(f'Podany termin jest zajęty przez przerwę, nie mogę dodać lekcji {lesson.name}.')
